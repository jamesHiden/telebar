...
if (!connectionString) {
  throw new Error('POSTGRES_URL یا DATABASE_URL تنظیم نشده. فایل .env.local را بررسی کنید.');
}
...
export async function listActiveProducts() {
  const { rows } = await query('SELECT * FROM products WHERE active ORDER BY name');
  return rows;
}
...
export async function upsertProduct(name, price, availableQty, unit = 'کیلوگرم') {
  const { rows } = await query(
    `INSERT INTO products (name, unit, price, available_qty, active, updated_at)
